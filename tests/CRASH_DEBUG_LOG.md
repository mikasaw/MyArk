...
## 2026-09-20 — 队列 4：win32k 会话基址多候选交叉校验（0x772/0x773/0x774 共用解析器加固）

### 现象
- 无线上故障——这是预防性加固：SystemModuleInformation 里会话驱动
  三件套**按会话各有一条**，多会话主机上"盲取第一条 win32kbase"
  可能拿到**别的会话**的副本；该副本的会话 VA 在调用者地址空间
  不映射，后续守卫读全失败 → 整个内核面（内核表/堆基址/定时器/
  事件钩子）退化为 informational，交互会话（s1）调用者首次中招。

### 与参考的对比
- 旧实现：FindSessionImage 取第一个同名条目 → ResolveSessionBase
  只验证这一个。会话排序是"boot 时序偶然"（s0 先加载所以 s0 调用者
  恰好命中），s1 调用者从未被测试覆盖。
- 新实现：FindSessionImageCandidates 收集**全部**候选（去重、cap 8，
  stage 5=找不到），三个消费端各自按内容验证逐个尝试：
  0x772 用 gSharedInfo 三字段+heSize 交叉校验；0x773/0x774 用
  表头/链头读成功且值 0-或-canonical 判定；stage 6 = 有候选但全不
  合。

### 修复尝试
- 纯解析器重构（3 个消费端同步改造），无新 IOCTL/协议变化；
  双 guest 部署回归 [VERIFY] OK。

### 关键决策回顾
- **用"内容可读性"当会话过滤器**：外会话 VA 在调用者地址空间必然
  未映射（会话空间按进程页表切换），守卫读天然失败——不需要显式
  的 session-id 映射管道，验证即过滤。
- 已知边界：候选遍历顺序仍按模块表序；s1 交互调用者路径无法在
  无人值守 VM 冒烟中覆盖（schtasks 三路全堵，见 R3-10b-i 记录），
  留值守窗口验证。

### TODO
- s1 交互调用者端到端验证（需值守/交互会话投递方案）。
