...
| R3-10b | win32k 定时器 + 内核表 + 堆基址 ✅（2026-09-19/20） | 0x772 内核 gSharedInfo/aheList（R3-10b-ii）；0x773 TIMER 走查 （64 桶 0xA0 节点，1903 标定 / 22631 门控，R3-10b-iii）；0x772 行级内核对象指针 = W32P 候选扫描 + 每行 hdr 自检 （R3-10b-iv，1903/22631 双机严格认证，PsiMatch bit1=实证） | 1 天 |
| R3-10c | WinEvent 钩子枚举 ✅（2026-09-20） | 0x774：win32kbase!gpWinEventHooks 单链走查（LIFO，next@+0x18），行含句柄(type15)/EventMin/Max/内部flags/用户回调/idProcess/idThread；1903 RVA 0x219200 标定 + 22631 门控；verify marker 三钩子往返双绿；CLI win32k event-hooks | 0.5 天 |
