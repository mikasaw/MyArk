"""
wfp R3 - CLI subcommands.

Usage:
    myark-cli wfp enumerate-callouts
"""
...
def _cmd_enumerate(_args: argparse.Namespace) -> int:
    ...
    try:
        r = P.enumerate_callouts(client)
        print(f"# wfp enumerate-callouts: source={r.source} count={r.count}")
        return 0
    finally:
...
def _setup_cli(subparsers, _client=None) -> None:
    p_root = subparsers.add_parser(
        "wfp",
        help="wfp module (WFP callout enumeration; stub for S7.3)",
    )
    p_root.set_defaults(_handler=_cmd_root)

    p_subs = p_root.add_subparsers(dest="wfp_subcommand", required=True)

    p1 = p_subs.add_parser("enumerate-callouts", help="enumerate WFP callouts")
    p1.set_defaults(_handler=_cmd_enumerate)


def _cmd_root(_args: argparse.Namespace) -> int:
    print("!! wfp: missing subcommand", file=sys.stderr)
    return 2
